...
public class BukkitArc extends JavaPlugin {
	...
	public void onEnable() {
		this.l.info("ArcPlug has been enabled!");
		new ListenerClass(this);
	}

	public void onDisable() {
		this.l.info("Disabling ArcPlug!");
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label,
		...
		if (cmd.getName().equalsIgnoreCase("greylist")
			...
			if (p.hasPermission("arcane.trusted"))
				...
				p.sendMessage(gold + "[ArcaneSurvival] " + white + "You are "
						+ red + "not" + white + " on the greylist!");
				p.sendMessage(gold + "[ArcaneSurvival] " + white
		...
		if (cmd.getName().equalsIgnoreCase("username")) {
			...
			p.sendMessage(gold + "[ArcaneSurvival] " + gray
					+ p.getDisplayName() + white + " is your username.");
		...
		if (cmd.getName().equalsIgnoreCase("forum")) {
			...
			p.sendMessage("");
			p.sendMessage(white + "     http://www.arcane-minecraft.com/forum");
			p.sendMessage("");
		...
		if (cmd.getName().equalsIgnoreCase("one")
			...
			p.sendMessage("The magic which allowed you to spawn fireworks from thin air has gone.");
		...
		if (cmd.getName().equalsIgnoreCase("dclem")) {
			...
			if (args.length == 0) {
				...
				p.sendMessage(intro + gold + bold
						+ "DClem's Anti-Boredom Build Ideas!" + red + bold
						+ " V1.0");
				p.sendMessage(intro + gray + "For " + yellow
			...
			if (args.length == 1) {
				...
				if (args[0].equalsIgnoreCase("small")) {
					...
					p.sendMessage(intro + gold + "DClem's Build Idea: " + gray
							+ smallrandom);
		...
		if (cmd.getName().equalsIgnoreCase("help") && args.length != 1) {
			...
			p.sendMessage(gray + bold + " ----- " + gold + bold + "Arcane Help"
					+ gray + bold + " ----- ");

			p.sendMessage(gray + " " + gold + "/spawn " + gray
					+ "- Return to the spawn");
			p.sendMessage(gray + " " + gold + "/home " + gray
					+ "- Takes you to your home");
			p.sendMessage(gray + " " + gold + "/pvp " + gray
					+ "- Toggle PvP combat");
			p.sendMessage(gray + " " + gold + "/seen" + gray
					+ " - Seen commands");
			p.sendMessage(gray + " " + gold + "/help lwc " + gray
			...
			if (p.hasPermission("arcane.mod")) {
				...
				p.sendMessage(gray + " " + gold + "/help mods" + gray
						+ " - Moderator commands");
			}
		...
		if (args.length == 1) {
			...
			if (args[0].equalsIgnoreCase("lwc")) {
				p.sendMessage(gray + bold + " ----- " + gold + bold
						+ "Help LWC" + gray + bold + " ----- ");
				p.sendMessage(gray + "> " + gold + "/cprivate " + gray
						+ "- Create a protection");
				p.sendMessage(gray + "> " + gold + "/cremove " + gray
						+ "- Remove a protection");
				p.sendMessage(gray + "> " + gold + "/cpublic " + gray
						+ "- Create a public protection");
				p.sendMessage(gray + "> " + gold + "/cinfo " + gray
						+ "- View info on a protection");
				p.sendMessage(gray + "> " + gold + "/cmodify <player> " + gray
						+ "- Add a player to a protection");
				p.sendMessage(gray + "> " + gold + "/lwc flag hopper on" + gray
			...
			if (args[0].equalsIgnoreCase("mods")
				...
				p.sendMessage(gray + bold + " ----- " + gold + bold
						+ "Help Mods" + gray + bold + " ----- ");
				p.sendMessage(gray + "> " + gold + "/ban " + gray
						+ "- Ban a player" + red + it + "");
				p.sendMessage(gray + "> " + gold + "/kick " + gray
						+ "- Kick a player");
				p.sendMessage(gray + "> " + gold + "/mute " + gray
						+ "- Mute a player");
				p.sendMessage(gray + "> " + gold + "/frz " + gray
						+ "- Freze a player" + red + it + " Agent's favorite");
				p.sendMessage(gray + "> " + gold + "/tp <player> " + gray
						+ "- Teleport to a player");
				p.sendMessage(gray + "> " + gold + "/tp <X Y Z> " + gray
			...
			if (args[0].equalsIgnoreCase("mods2")
				...
				p.sendMessage(gray + "> " + gold + "/vanish " + gray
						+ "- Vanish yourself");
				p.sendMessage(gray + "> " + gold + "/reappear " + gray
						+ "- Turn visible");
				p.sendMessage(gray + "> " + gold + "/co i " + gray
						+ "- Enable the inspection wand");
				p.sendMessage(gray + "> " + gold + "/co l <params> " + gray
						+ "- Perform a lookup");
				p.sendMessage(gray + "> " + gold + "/open " + gray
						+ "- Open an inventory");
				p.sendMessage(gray + "> " + gold + "/a <msg> " + gray
						+ "- Staff chat");
			...
			if (args[0].equalsIgnoreCase("mods3")
				...
				p.sendMessage(gray + "> " + gold + "/whitelist " + gray
						+ "- Display whitelist commands");
				p.sendMessage(gray + "> " + gold + "/ail " + gray
						+ "- Display IP lookup commands, bra.");
				p.sendMessage(gray + "> " + gold
						+ "/pex group trusted user add <user> " + gray
						+ "- Greylist command");
				p.sendMessage(gray + "> " + gold + "/ultraban " + gray
